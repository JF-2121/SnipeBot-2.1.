...
export type Category = "polos" | "pants" | "shoes";
...
export interface CategoryChannelConfig {
  ...
  channelId: string;
  keywords: string[];
  /** If any of these appear, this category is skipped even if a keyword also matched. */
...
export const CATEGORY_CHANNELS: CategoryChannelConfig[] = [
  {
    category: "shoes",
    label: "Shoes",
 ...
 */
export function classifyCategory(title: string): CategoryChannelConfig | undefined {
  const normalized = String(title || "").toLowerCase();
  if (!normalized) return undefined;
  return CATEGORY_CHANNELS.find((config) => {
    if (!config.keywords.some((kw) => normalized.includes(kw))) return false;
    if (config.excludeKeywords?.some((kw) => normalized.includes(kw))) return false;
    return true;
  });
}
