 ...
 */
async function scrapeViaOneShotPython(searchText, maxPrice) {
    try {
...
async function scrapeViaDaemon(searchText, maxPrice) {
    if (daemonPermanentlyFailed)
        return null;
    if (!daemonProcess) {
    ...
    return new Promise((resolve) => {
        ...
        const payload = { id, search_text: searchText };
        if (maxPrice && maxPrice > 0)
 ...
 */
export async function enrichSeller(sellerId) {
    if (!sellerId)
        return null;
    if (daemonPermanentlyFailed)
        return null;
